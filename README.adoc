...
For IDE-like completion I suggest to try
https://github.com/neoclide/coc.nvim[coc.nvim] plugin or
...
== Setting up YouCompleteMe for Godot ==

Set up YCM according to
https://github.com/ycm-core/YouCompleteMe#installation[it's documentation].
(i.e. install with package manager then run `./install.py`).

There are https://github.com/ycm-core/lsp-examples[instructions for enabling godot].
...
and hopefully would work for you. For other vim/neovim language server client
implementations setup consult their documentation.
